...
profileRouter.get('/profile/:userId', async (req, res) => {
  ...
  if (!userId) {
    return res
      .status(400)
      .json(sendErrorResponseForInvFields('userid not provided'));
  }
  try {
    ...
    if (!user) {
      return res
        .status(404)
        .json(
          sendCustomErrorResponse('user not found, provide correct userId'),
        );
    }
...
profileRouter.patch('/profile/password', userAuth, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(203)
      .json(sendSuccessBodyResponse('password changed successfully'));
  } catch (e) {
